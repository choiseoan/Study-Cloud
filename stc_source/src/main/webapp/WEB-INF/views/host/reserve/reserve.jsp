							...
							<c:forEach items="${reserveVo}" var="product">
								<tr>
									...
									<td class="text-center"><a href="#"
										onClick="moveReserveCheck('${product.regDate }', '${product.proNum }', '${product.memId }');">${product.proName}</a></td>
				...
				<div class="pagination">
					<ul>
						<c:choose>
							...
							<c:otherwise>
								<li><a class="pe-7s-angle-left"
									href="${pageContext.request.contextPath}/host/reserve?page=${pageInfo.currentPage - 1 }"></a>
								</li>
							</c:otherwise>
						</c:choose>
						<c:forEach begin="${pageInfo.startPage }"
							...
							<li><a
								href="${pageContext.request.contextPath}/host/reserve?page=${page }">${page }</a></li>
						</c:forEach>
						<c:choose>
							...
							<c:otherwise>
								<li><a class="pe-7s-angle-right"
									href="${pageContext.request.contextPath}/host/reserve?page=${pageInfo.currentPage +1 }"></a></li>
							</c:otherwise>
						</c:choose>
					</ul>
				</div>
